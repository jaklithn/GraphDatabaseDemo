...
using Cosmos.Movies.Business.Extensions;
...
namespace Cosmos.Movies.Business.Services
{
    public class CosmosRepository : IDisposable
    {
        ...
        public async Task LoadMovies(MovieContainer container)
        {
            ...
            {
                //var person = container.Persons.Single(p => p.TmdbId.ToString() == actorRelation.FromId);
                //var movie = container.Movies.Single(m => m.TmdbId.ToString() == actorRelation.ToId);
                //await AddVertex(movieCollection, person, "tmdbId");
                //await AddVertex(movieCollection, movie, "tmdbId");
